...
var NX_NOTIFY_ON_START_TOUCH					= (1<<1);	//!< Pair callback will be called when the pair starts to be in contact
var NX_NOTIFY_ON_END_TOUCH						= (1<<2);	//!< Pair callback will be called when the pair stops to be in contact
var NX_NOTIFY_ON_TOUCH							= (1<<3);	//!< Pair callback will keep getting called while the pair is in contact
var NX_NOTIFY_ON_IMPACT							= (1<<4);	//!< [Not yet implemented] pair callback will be called when it may be appropriate for the pair to play an impact sound
var NX_NOTIFY_ON_ROLL							= (1<<5);	//!< [Not yet implemented] pair callback will be called when the pair is in contact and rolling.
var NX_NOTIFY_ON_SLIDE							= (1<<6);	//!< [Not yet implemented] pair callback will be called when the pair is in contact and sliding (and not rolling).
var NX_NOTIFY_FORCES							= (1<<7);	//!< The (summed total) friction force and normal force will be given in the NxContactPair variable in the contact report.
var NX_NOTIFY_ON_START_TOUCH_FORCE_THRESHOLD	= (1<<8);	//!< Pair callback will be called when the contact force between two actors exceeds one of the actor-defined force thresholds
var NX_NOTIFY_ON_END_TOUCH_FORCE_THRESHOLD		= (1<<9);	//!< Pair callback will be called when the contact force between two actors falls below the actor-defined force thresholds
var NX_NOTIFY_ON_TOUCH_FORCE_THRESHOLD			= (1<<10);	//!< Pair callback will keep getting called while the contact force between two actors exceeds one of the actor-defined force thresholds
var NX_NOTIFY_CONTACT_MODIFICATION				= (1<<16);	//!< Generate a callback for all associated contact constraints, making it possible to edit the constraint. This flag is not included in NX_NOTIFY_ALL for performance reasons. \see NxUserContactModify

var NX_NOTIFY_ALL								= (NX_NOTIFY_ON_START_TOUCH|NX_NOTIFY_ON_END_TOUCH|NX_NOTIFY_ON_TOUCH|NX_NOTIFY_ON_IMPACT|NX_NOTIFY_ON_ROLL|NX_NOTIFY_ON_SLIDE|NX_NOTIFY_FORCES|
