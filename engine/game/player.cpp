...
void Player::do_rotation(float x, float y, float z)
{
	rot.x += x * mouse_sens_x;
	rot.y += y * mouse_sens_y;
...
void Player::do_move(int direction)
{
...
{
	D3DXVECTOR3 start_pos = actual_pos;
	
	move_left = 1.0f;
	move_slides = 0;

	process_moves();
	D3DXVECTOR3 end_pos = actual_pos;
...
void Player::do_step_interpol()
{
	...
	D3DXVECTOR3 step_start = actual_pos;
	D3DXVECTOR3 step_end = actual_pos;
	step_start.y = step_start_height;
	step_end.y = step_end.y;	
	D3DXVec3Lerp(&pos, &step_start, &step_end, step_interpol);
	//pos = actual_pos;
}
...
void Player::do_slide()
{	
	// avoid infinite recursion
	if(num_touched_brushes + 1 > MAX_SLIDES)
	{
		LOG("[Player::do_slide] WARNING: hit MAX_SLIDES");
		return;
	}
...
bool Player::do_stepDown()
{
	...
	step_interpol = 0.0f;
	in_step = true;
	step_end_height = actual_pos.y;
	
	return false;
}

bool Player::do_falling()
{
	D3DXVECTOR3 start_pos = actual_pos;
	D3DXVECTOR3 end_pos = actual_pos;
...
void Player::process_moves()
{
	D3DXVECTOR3 acceleration_vect(0, 0, 0);	

	if(moving[MOVE_UP]) {
		actual_pos.y += player_speed * ((float)timer::delta_ms / (1000.0f / 600.0f));
	}
