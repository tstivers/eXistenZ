...
namespace jscomponent
{
	...
	static JSFunctionSpec class_methods[] =
	{
		...
		JS_FN("setPos", WRAP_FASTNATIVE(PosComponent::setPos), 1, 0),
		JS_FN("getPos", WRAP_FASTNATIVE(PosComponent::getPos), 0, 0),
		JS_FN("setRot", (JSFastNativeCall<void(PosComponent::*)(const D3DXVECTOR3&), &PosComponent::setRot>), 1, 0),
		JS_FN("getRot", WRAP_FASTNATIVE(PosComponent::getRot), 0, 0),
		JS_FN("setScale", WRAP_FASTNATIVE(PosComponent::setScale), 1, 0),
		JS_FN("getScale", WRAP_FASTNATIVE(PosComponent::getScale), 0, 0),
		JS_FS_END
	};
}
